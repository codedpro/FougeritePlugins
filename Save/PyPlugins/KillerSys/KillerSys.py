...
class KillerSys:
    ...
    def On_PluginInit(self):
        config = self.KillerSysConfig()
        self.KillerSysname = config.GetSetting("Settings", "KillerSysname")
        ...
        Util.ConsoleLog("KillerSys by " + __author__ + " Version: " + __version__ + " loaded.", False)

    def On_Command(self, Player, cmd, args):
        if cmd == "uautoban":
            if len(args) == 0:
                Player.Message("---KillerSys " + __version__ + "---")
                Player.Message("/uautoban name - Unbans player")
            else:
                if not Player.Admin and not Player.Moderator:
                    Player.MessageFrom(self.KillerSysname, "You aren't an admin!")
                    return
                ...
                if not b:
                    ...
                    return
                Player.MessageFrom(self.KillerSysname, "Player " + pl + " unbanned!")
        elif cmd == "flushKillerSys":
            if Player.Admin:
                DataStore.Flush("KillerSysAVG")
                DataStore.Flush("KillerSysAVG2")
                Player.MessageFrom(self.KillerSysname, "Flushed!")
        ...
        if DeathEvent.DamageType is not None and DeathEvent.Victim is not None and DeathEvent.Attacker is not None and \
            ...
            if DeathEvent.AttackerIsNPC:
                if self.ean == 1:
                    ...
                    Server.BroadcastFrom(self.KillerSysname, a)
                return
            ...
            if self.WasSuicide(id, vid):
                if self.esn == 1:
                    ...
                    Server.BroadcastFrom(self.KillerSysname, n)
                return
            ...
            if bleed == "Bullet":
                ...
                if str(bodyPart) == "Head":
                    if DataStore.Get("KillerSysAVG", DeathEvent.Attacker.UID) is None:
                        DataStore.Add("KillerSysAVG", DeathEvent.Attacker.UID, 1)
                        c = 1
                    else:
                        c = DataStore.Get("KillerSysAVG", DeathEvent.Attacker.UID) + 1
                        DataStore.Add("KillerSysAVG", DeathEvent.Attacker.UID, c)
                else:
                    if DataStore.Get("KillerSysAVG2", DeathEvent.Attacker.UID) is None:
                        DataStore.Add("KillerSysAVG2", DeathEvent.Attacker.UID, 1)
                    else:
                        cd = DataStore.Get("KillerSysAVG2", DeathEvent.Attacker.UID) + 1
                        DataStore.Add("KillerSysAVG2", DeathEvent.Attacker.UID, cd)
                if c >= 5:
                    cd = DataStore.Get("KillerSysAVG2", DeathEvent.Attacker.UID)
                    if cd is not None:
                        calc = round(c / cd, 2)
                        n = n + " (HAvg: " + str(calc) + "% )"
                Server.BroadcastFrom(self.KillerSysname, n)
                if self.autoban == 1:
                    ...
                    if distance > self.RangeOf(weapon) > 0:
                        ...
                        if (tpfriendteleport == "none" or tpfriendteleport is None) and \
                            ...
                            if distance >= 1000:
                                return
                            Server.BroadcastFrom(self.KillerSysname, self.red + z)
                            self.Log(killer, weapon, distance, victim, bodyPart, damage, 1,
                                     str(killerloc), str(location))
                            DataStore.Add("KillerSysBAN", vid, str(location))
                            ...
                            if distance >= 1000:
                                return
                            Server.BroadcastFrom(self.KillerSysname, t)
                ...
                if weapon == "Hunting Bow":
                    ...
                    Server.BroadcastFrom(self.KillerSysname, hn)
                    if self.autoban == 1:
                        if distance > self.RangeOf(weapon) > 0:
                            tpfriendteleport = DataStore.Get("tpfriendautoban", id)
                            hometeleport = DataStore.Get("homesystemautoban", id)
                    ...
                    Server.BroadcastFrom(self.KillerSysname, s)
                ...
                Server.BroadcastFrom(self.KillerSysname, x)
            elif bleed == "Bleeding":
                n = self.bleeding
                n = n.replace("victim", victim)
                n = n.replace("killer", killer)
    ...
    def On_PlayerSpawned(self, Player, SpawnEvent):
        id = Player.SteamID
        if DataStore.ContainsKey("KillerSysBAN", id):
            get = DataStore.Get("KillerSysBAN", id)
            loc = self.Replace(get)
            newloc = Util.CreateVector(float(loc[0]), float(loc[1]), float(loc[2]))
            Player.TeleportTo(newloc)
            Player.MessageFrom(self.KillerSysname, self.green + "You got teleported back where you died!")
            DataStore.Remove("KillerSysBAN", id)
    ...
    def KillerSysConfig(self):
        return Plugin.GetIni("KillerSysConfig")
